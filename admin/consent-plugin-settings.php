 ...
 * @subpackage Consent_Cookies/admin
 ...
 * @since      1.0.0
 *
 * @package    Wpcc_Consent_
 * @subpackage Wpcc_Consent_/admin
 */
/**
 * Class WordPress_Plugin_Template_Settings
 *
 */
class Wppb_Admin_Settings {
    /**
...
ul.banners.footer .seventy-five-width {
    ...
    public function general_options_callback() {
        $options = get_option('wpcc_consent_display_options');
        var_dump($options);
    ...
    public function social_options_callback() {
        $options = get_option('wpcc_consent_social_options');
        var_dump($options);
    ...
    public function input_examples_callback() {
        $options = get_option('wpcc_consent_input_examples');
        var_dump($options);
    ...
    public function initialize_display_options() {
        ...
        if( false == get_option( 'wpcc_consent_display_options' ) ) {
            $default_array = $this->default_display_options();
            add_option( 'wpcc_consent_display_options', $default_array );
        }
        add_settings_section(
            ...
            'wpcc_consent_display_options'		                // Page on which to add this section of options
        );
        ...
        add_settings_field(
            ...
            'wpcc_consent_display_options',	            // The page on which this option will be displayed
    ...
    public function initialize_social_options() {
        delete_option('wpcc_consent_social_options');
        if( false == get_option( 'wpcc_consent_social_options' ) ) {
            $default_array = $this->default_social_options();
            update_option( 'wpcc_consent_social_options', $default_array );
        } // end if
        add_settings_section(
            ...
            'wpcc_consent_social_options'		// Page on which to add this section of options
        );
        add_settings_field(
            'twitter',
            'Twitter',
            array( $this, 'twitter_callback'),
            'wpcc_consent_social_options',
    ...
    public function initialize_input_examples() {
        //delete_option('wpcc_consent_input_examples');
        if( false == get_option( 'wpcc_consent_input_examples' ) ) {
            $default_array = $this->default_input_options();
            update_option( 'wpcc_consent_input_examples', $default_array );
        } // end if
        add_settings_section(
            'input_examples_section',
            __( 'Input Examples', 'wppb-demo-plugin' ),
            array( $this, 'input_examples_callback'),
            'wpcc_consent_input_examples'
        );
        add_settings_field(
            'Input Element',
    ...
    public function toggle_header_callback($args) {
        ...
        $html = '<input type="checkbox" id="show_header" name="wpcc_consent_display_options[show_header]" value="1" ' . checked( 1, isset( $options['show_header'] ) ? $options['show_header'] : 0, false ) . '/>';
    ...
    public function toggle_content_callback($args) {
        $options = get_option('wpcc_consent_display_options');
        $html = '<input type="checkbox" id="show_content" name="wpcc_consent_display_options[show_content]" value="1" ' . checked( 1, isset( $options['show_content'] ) ? $options['show_content'] : 0, false ) . '/>';
        $html .= '<label for="show_content">&nbsp;'  . $args[0] . '</label>';
        echo $html;
    } // end toggle_content_callback
    public function toggle_footer_callback($args) {
        $options = get_option('wpcc_consent_display_options');
        $html = '<input type="checkbox" id="show_footer" name="wpcc_consent_display_options[show_footer]" value="1" ' . checked( 1, isset( $options['show_footer'] ) ? $options['show_footer'] : 0, false ) . '/>';
    ...
    public function twitter_callback() {
        ...
        // Render the output
        echo '<input type="text" id="twitter" name="wpcc_consent_social_options[twitter]" value="' . $url . '" />';
    ...
    public function facebook_callback() {
        ...
        // Render the output
        echo '<input type="text" id="facebook" name="wpcc_consent_social_options[facebook]" value="' . $url . '" />';
    ...
    public function googleplus_callback() {
        ...
        // Render the output
        echo '<input type="text" id="googleplus" name="wpcc_consent_social_options[googleplus]" value="' . $url . '" />';
    } // end googleplus_callback
    public function input_element_callback() {
        $options = get_option( 'wpcc_consent_input_examples' );
        // Render the output
        echo '<input type="text" id="input_example" name="wpcc_consent_input_examples[input_example]" value="' . $options['input_example'] . '" />';
    } // end input_element_callback
    public function textarea_element_callback() {
        $options = get_option( 'wpcc_consent_input_examples' );
        // Render the output
        echo '<textarea id="textarea_example" name="wpcc_consent_input_examples[textarea_example]" rows="5" cols="50">' . $options['textarea_example'] . '</textarea>';
    } // end textarea_element_callback
    public function checkbox_element_callback() {
        $options = get_option( 'wpcc_consent_input_examples' );
        $html = '<input type="checkbox" id="checkbox_example" name="wpcc_consent_input_examples[checkbox_example]" value="1"' . checked( 1, $options['checkbox_example'], false ) . '/>';
        $html .= '&nbsp;';
    ...
    public function radio_element_callback() {
        $options = get_option( 'wpcc_consent_input_examples' );
        $html = '<input type="radio" id="radio_example_one" name="wpcc_consent_input_examples[radio_example]" value="1"' . checked( 1, $options['radio_example'], false ) . '/>';
        $html .= '&nbsp;';
        $html .= '<label for="radio_example_one">Option One</label>';
        $html .= '&nbsp;';
        $html .= '<input type="radio" id="radio_example_two" name="wpcc_consent_input_examples[radio_example]" value="2"' . checked( 2, $options['radio_example'], false ) . '/>';
        $html .= '&nbsp;';
    ...
    public function select_element_callback() {
        $options = get_option( 'wpcc_consent_input_examples' );
        $html = '<select id="time_options" name="wpcc_consent_input_examples[time_options]">';
